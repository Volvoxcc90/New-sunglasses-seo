...
def fill_wb_template(
    ...
    end_row = min(ws.max_row, start_row + max(0, int(rows_to_fill)) - 1)

    total_rows = end_row - start_row + 1
    if total_rows <= 0:
        raise RuntimeError("Нет строк для заполнения (после заголовка / в пределах rows_to_fill)")
    ...
    stem = Path(input_xlsx).stem
    suffix = f"_ready_{out_index}.xlsx" if int(out_index) > 0 else "_ready.xlsx"
    out_path = str(Path(input_xlsx).with_name(stem + suffix))
    wb.save(out_path)
